...
# stream produced by scripts/replay/replay_daemon.py.
#
...
#   EPOCHS=100 \
#   bash scripts/train/launch_with_external_replay.sh
#
...
MOVABLE_MODE="${MOVABLE_MODE:-surface_ads}"
...
echo "[train] MOVABLE_MODE=${MOVABLE_MODE}"
...
setsid ${MICROMAMBA} run -n adsorbgen python -m adsorbgen.training.train_cli \
  --arch v1 \
  --train-lmdb \
