...
OUT_ROOT="${OUT_ROOT:-/home1/irteam/data/replay/diag_replay_setting_ood50_strict_valid_base_random_adsorbdiff_$(date +%Y%m%d_%H%M%S)}"
...
run_one() {
  ...
  mkdir -p "${out}/logs"
  echo "[strict-valid] start label=${label} mode=${mode} $(date -Is)" | tee -a "${OUT_ROOT}/logs/launcher.log"
  ...
  for shard in $(seq 0 $((NUM_SHARDS - 1))); do
    ...
    echo "[strict-valid] ${label} shard=${shard} gpu=${gpu} pid=${pids[-1]}" | tee -a "${OUT_ROOT}/logs/launcher.log"
    sleep 2
  done
  ...
  if [[ "${failed}" != "0" ]]; then
    echo "[strict-valid] ERROR label=${label}; logs=${out}/logs" >&2
  ...
  echo "[strict-valid] done label=${label} $(date -Is)" | tee -a "${OUT_ROOT}/logs/launcher.log"
}

run_one "base_ID_pairdist_epoch149_strict_valid" "model" "${BASE_CKPT}"
run_one "B2_random_heuristic_strict_valid" "random"
run_one "B3_adsorbdiff_strict_valid" "adsorbdiff"

echo "[strict-valid] all done OUT_ROOT=${OUT_ROOT}" | tee -a "${OUT_ROOT}/logs/launcher.log"
