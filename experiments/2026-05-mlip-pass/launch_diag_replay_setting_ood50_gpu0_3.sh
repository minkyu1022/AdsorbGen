...
OUT_ROOT="${OUT_ROOT:-/home1/irteam/data/replay/diag_replay_setting_ood50_$(date +%Y%m%d_%H%M%S)}"
...
run_one() {
  ...
  mkdir -p "${out}/logs"
  echo "[diag] start label=${label} mode=${mode} $(date -Is)"
  ...
  for shard in 0 1 2 3; do
    ...
    (
      cd "${REPO}"
      CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON_BIN}" "${SCRIPT}" "${args[@]}"
    ) >"${log}" 2>&1 &
    pids+=("$!")
    echo "$!" > "${out}/logs/pid_shard${shard}.txt"
    echo "[diag] ${label} shard=${shard} gpu=${gpu} pid=$!"
    sleep 2
  done
  ...
  if [[ "${failed}" != "0" ]]; then
    echo "[diag] FAILED label=${label}; see ${out}/logs" >&2
    return 1
  fi
  "${PYTHON_BIN}" "${MERGE}" "${out}" | tee "${out}/logs/merge.log"
  echo "[diag] done label=${label} $(date -Is)"
}

run_one "ID_pairdist_epoch149" "model" "/home1/irteam/runs/training/ID_mlip_pairdist_only_1x_bs64_expand_20260521_213544/last.ckpt"
run_one "x1_LP_102M_epoch099" "model" "/home1/irteam/runs/training/x1_LP_102M/ckpt_epochepoch=099.ckpt"
run_one "B1_adsorbate_only_pair_dist" "model" "/home1/irteam/runs/B1_adsorbate_only_pair_dist_loss/last.ckpt"
run_one "baseline2_random_heuristic" "random"
...
echo "[diag] all done OUT_ROOT=${OUT_ROOT}"
