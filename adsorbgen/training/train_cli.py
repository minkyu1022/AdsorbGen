...
Usage:
    ...
    # Single-GPU smoke
    PYTHONPATH=AdsorbGen python -m adsorbgen.training.train_cli \
    ...
    PYTHONPATH=AdsorbGen python -m adsorbgen.training.train_cli --devices 4 ...
"""
...
from adsorbgen.data.dataset import (  # noqa: E402
    MixedReplayDataset, PlacementPriorDataset, PreprocessedDisplacementDataset,
    collate_displacement,
)
from adsorbgen.evaluation.metrics import compute_anomaly_metrics, compute_displacement_metrics  # noqa: E402
from adsorbgen.replay.eval import (  # noqa: E402
...
from adsorbgen.models.variants import get_variant, list_variants  # noqa: E402
...
class AdsorbGenModule(L.LightningModule):
    ...
    def _effective_movable_mask(self, batch):
        mode = str(getattr(self.hparams, "movable_mode", "surface_ads"))
    ...
    def _accumulate_sample_eval(self, batch, src: str):
        ...
        movable = self._effective_movable_mask(batch)
        ...
        x_out = euler_sample(
            ...
            movable, batch["pad_mask"], self.flow_cfg,
...
def main():
    ...
    p.add_argument("--movable-mode", choices=["surface_ads", "adsorbate_only"],
                   default="surface_ads",
                   help="which atoms are integrated/lossed by the flow. "
                        "surface_ads matches the default OC20 movable surface+adsorbate mask; "
                        "adsorbate_only freezes the slab at x0 and moves only tag==2 atoms.")
    ...
    p.add_argument("--external-replay-dir", type=str, default="",
                   help="consume success entries written by an external replay daemon "
                        "(scripts/replay/replay_daemon.py). Mutually exclusive with --use-replay.")
    ...
    from adsorbgen.models.factory import build_model as _bm
    _m = _bm(model_cfg)
