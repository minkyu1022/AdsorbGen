...
from adsorbgen.replay.buffer import ReplayBuffer, ReplayEntry
from adsorbgen.replay.viz import (
...
def _passes_anomaly(
    ...
    Delegates to ``adsorbgen.evaluation.metrics._score_record_anomaly`` so replay shares the
    *same* anomaly definition as the sample_eval/validation pipeline:
    ...
    """
    from adsorbgen.evaluation.metrics import _score_record_anomaly
    record = {
        "sid": int(sid),
        "ads_id": int(ads_id),
...
def _runtime_movable_mask(model, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    raise ValueError(f"Unknown adsorbgen_movable_mode={mode!r}")
...
def run_replay_eval(
    ...
    for start in flow_pbar:
        ...
        _es = euler_sample(
            fwd, batch["pos"],
            movable, batch["pad_mask"], flow_cfg,
            num_steps=cfg.flow_steps,
            use_sde=cfg.use_sde,
            refine_final=cfg.refine_final,
    ...
    metrics = {
        ...
        "refine_final": bool(cfg.refine_final),
    }
    if all_preds is not None:
